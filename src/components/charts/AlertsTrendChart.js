...
const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
        return (
            <div className="p-2 bg-surface/90 backdrop-blur-sm border border-ui-border rounded-lg">
                <p className="text-sm text-text-primary">{`${payload[0].value} Alerts`}</p>
            </div>
        );
    }
    return null;
      ...
      {!minimal && (
        <div className="flex justify-between items-center mb-1">
          <div>
            ...
            <p className="text-xs text-text-secondary">Last 60 minutes</p>
          </div>
